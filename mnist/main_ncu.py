...
def train(args, model, device, train_loader, optimizer, the_number_of_epochs):
    model.train()
    for index_batch, (data, target) in enumerate(train_loader):
        data, target = data.to(device), target.to(device)
        optimizer.zero_grad()
        output = model(data, the_number_of_epochs, index_batch)
        loss = F.nll_loss(output, target)
        loss.backward()
        optimizer.step()
        if index_batch % args.log_interval == 0:
            print("Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}".format(
                the_number_of_epochs, index_batch * len(data), len(train_loader.dataset),
                100. * index_batch / len(train_loader), loss.item()))
            if args.dry_run:
                break
...
def main():
    ...
    parser.add_argument("--the_number_of_epochss", type=int, default=14, metavar="N",
                        help="number of the_number_of_epochss to train (default: 14)")
